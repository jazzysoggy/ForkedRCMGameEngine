...
local pressed1 = false -- Determines if the button was pressed before
...
--			local acsSettings = require(v:FindFirstChild("ACS_Settings"))
--			if acsSettings.WeaponWeight then
--				weight += acsSettings.WeaponWeight
--			end
--			if acsSettings.AmmoWeight then
--				if typeof(acsSettings.StoredAmmo) == "table" then
--					weight += #acsSettings.StoredAmmo * acsSettings.AmmoWeight
--				elseif acsSettings.MagCount then
--					weight += math.round(acsSettings.StoredAmmo / acsSettings.AmmoInGun * acsSettings.AmmoWeight)	
--				else
--					weight += acsSettings.StoredAmmo * acsSettings.AmmoWeight
--				end
--			end
--		end
--	end
--	weight += ACS_Client.Protecao.VestWeight.Value
--	weight += ACS_Client.Protecao.HelmetWeight.Value
--end

--ACS_Client.Protecao.VestWeight.Changed:Connect(calculateWeight)
--ACS_Client.Protecao.HelmetWeight.Changed:Connect(calculateWeight)
--plr.Backpack.ChildAdded:Connect(calculateWeight)
--plr.Backpack.ChildRemoved:Connect(calculateWeight)
--calculateWeight()
...
function SetWalkSpeed(Speed)
	...
	if gameRules.WeaponWeight and WeaponInHand and WeaponData.WeaponWeight then
		local carrying = ACS_Client.Protecao.WeightCarrying.Value
		if script.Parent:GetAttribute("Injured") then
			carrying = carrying / 1.5
		end

		--local normalWeight = weight + WeaponData.WeaponWeight * 1.5

		--if WeaponData.AmmoWeight then
		--	if typeof(WeaponData.StoredAmmo) == "table" then
		--		normalWeight += #WeaponData.StoredAmmo * WeaponData.AmmoWeight
		--	elseif WeaponData.MagCount then
		--		normalWeight += math.round(WeaponData.StoredAmmo / WeaponData.AmmoInGun * WeaponData.AmmoWeight)	
		--	else
		--		normalWeight += WeaponData.StoredAmmo * WeaponData.AmmoWeight
		--	end
		--end

		--local weightDifference = normalWeight - carrying
		--local speedDecrease = Speed / (normalWeight / (carrying / 2))

		--if Speed - speedDecrease < 1 then
		--	char:WaitForChild("Humanoid").WalkSpeed = 1
		--elseif weightDifference < 0 then
		--	char:WaitForChild("Humanoid").WalkSpeed = Speed
		--else
		--	char:WaitForChild("Humanoid").WalkSpeed = Speed - speedDecrease			
		--end

		if Speed - (WeaponData.WeaponWeight - carrying) / (carrying / 2) * Speed < 1 then
			char:WaitForChild("Humanoid").WalkSpeed = 1
		elseif (WeaponData.WeaponWeight - carrying) / (carrying / 2) * Speed > 0 then
			char:WaitForChild("Humanoid").WalkSpeed = Speed - (WeaponData.WeaponWeight - carrying) / (carrying / 2) * Speed
		else
			char:WaitForChild("Humanoid").WalkSpeed = Speed			
		end
	else
...
function handleAction(actionName, inputState, inputObject)
	...
	if actionName == "Reload" and inputState == Enum.UserInputState.Begin and AnimDebounce and not CheckingMag and not reloading then
		if not pressed then
			...
			if typeof(StoredAmmo) == "table" then
				if pressed and tick() - last_press < 0.2 and not WeaponData.Jammed and Ammo > 0 and #StoredAmmo > 1 then -- or whatever time you want
					-- Your code
				...
				if pressed and tick() - last_press < 0.2 and not WeaponData.Jammed and Ammo > 0 then -- or whatever time you want
					-- Your code
					pressed = false -- After all your code, to reuse 	
					print("QuickReload")
					QuickReload = true
		...
		--elseif actionName == "Reload" and inputState == Enum.UserInputState.Begin and reloading and not WeaponData.ShellInsert then
		--	CancelReload = true			
	end
	...
	if actionName == "ADS" and inputState == Enum.UserInputState.Begin and AnimDebounce then
		if WeaponData and WeaponData.canAim and GunStance > -2 and not runKeyDown and not CheckingMag then
			aimming = not aimming
			if not aimming and breathHolding == true then
				TS:Create(cam,AimTween,{FieldOfView = cam.FieldOfView + 10}):Play()			
				breathHolding = false
			end
			ADS(aimming)
		end
	...
	if actionName == "Run" and not running and aimming and inputState == Enum.UserInputState.Begin and  ACS_Client.Variaveis.Stamina.Value > 0 then

		breathHolding = true

		TS:Create(cam,AimTween,{FieldOfView = cam.FieldOfView - 10}):Play()

	elseif actionName == "Run" and aimming and inputState == Enum.UserInputState.End and breathHolding then

		breathHolding = false

		TS:Create(cam,AimTween,{FieldOfView = cam.FieldOfView + 10}):Play()

	elseif actionName == "Run" and not reloading and inputState == Enum.UserInputState.Begin and running and not script.Parent:GetAttribute("Injured") and (ACS_Client.Variaveis.Stamina.Value > 100 or not gameRules.EnableStamina) then
		mouse1down = false		
		runKeyDown 	= true
		Stand()
		Stances = 0
		...
		CameraY = 0
		Lean()
		TS:Create(cam,AimTween,{FieldOfView = cam.FieldOfView + 5}):Play()		
		if breathHolding then
			breathHolding = false
			TS:Create(cam,AimTween,{FieldOfView = cam.FieldOfView + 10}):Play()			
		end
...
function setup(Tool)
	...
	if WeaponInHand.Handle:FindFirstChild("Muzzle") then
		if UnderBarrelAtt then
			if UnderBarrelAtt:FindFirstChild("Muzzle", true) then
				gunSize = math.max(math.abs(WeaponInHand.Handle:FindFirstChild("Muzzle").Position.Z), math.abs(UnderBarrelAtt:FindFirstChild("Muzzle", true).Position.Z))
			else
				gunSize = math.abs(WeaponInHand.Handle:FindFirstChild("Muzzle").Position.Z)						
			end
		else
			gunSize = math.abs(WeaponInHand.Handle:FindFirstChild("Muzzle").Position.Z)			
		end
	...
	StoredAmmo = WeaponData.StoredAmmo -- RepValues.StoredAmmo.Value
	CurAimpart = WeaponInHand:FindFirstChild("AimPart")
...
function CastAndFind (result, raycastParams, dir)
...
function CalculateDisatance (p1, p2)
...
function CastRay(Bullet, Origin)
	...
	while Bullet do
		...
		if raycastResult then
			...
			if Hit2 and Hit2:GetAttribute("PenCo") then

				local raycastParamsWhitelist = RaycastParams.new()
				local OnlyModel = {Hit2}
				raycastParamsWhitelist.FilterDescendantsInstances = OnlyModel
				raycastParamsWhitelist.FilterType = Enum.RaycastFilterType.Whitelist
				raycastParamsWhitelist.IgnoreWater = true
				...
				local secondPosition = CastAndFind(raycastResult, raycastParams, Direction)
				local distance = CalculateDisatance(raycastResult.Position, secondPosition.Position)
				if WeaponData.BulletPenetration - Hit2:GetAttribute("PenCo")*distance - Bullet:GetAttribute("DamageMultiplier") > 0 then
					table.insert(Ignore_Model, Hit2)
					recast = true
					Bullet:SetAttribute("DamageMultiplier", Bullet:GetAttribute("DamageMultiplier") + Hit2:GetAttribute("PenCo")*distance) 
					local BulletCF 	= CFrame.new(secondPosition.Position, Direction) 
					Bullet.CFrame = BulletCF
					local BulletMass = Bullet:GetMass()
					local impulseApplied = Bullet.AssemblyLinearVelocity*((Hit2:GetAttribute("PenCo")*distance)/(WeaponData.BulletPenetration - Bullet:GetAttribute("DamageMultiplier"))) * BulletMass
					if(impulseApplied.Magnitude > (Bullet.AssemblyLinearVelocity*BulletMass).Magnitude) then
						recast = false
						break
					end
					HitMod.HitEffect(Ignore_Model, raycastResult.Position, raycastResult.Instance , raycastResult.Normal, raycastResult.Material, WeaponData, Bullet:GetAttribute("DamageMultiplier"))
					Evt.HitEffect:FireServer(raycastResult.Position, raycastResult.Instance, raycastResult.Normal, raycastResult.Material, WeaponData, Bullet:GetAttribute("DamageMultiplier"))					
					HitMod.HitEffect(Ignore_Model, secondPosition.Position, secondPosition.Instance , secondPosition.Normal, secondPosition.Material, WeaponData, Bullet:GetAttribute("DamageMultiplier"))
					Evt.HitEffect:FireServer(secondPosition.Position, secondPosition.Instance, secondPosition.Normal, secondPosition.Material, WeaponData, Bullet:GetAttribute("DamageMultiplier"))				
					Bullet:ApplyImpulse(-Bullet.AssemblyLinearVelocity*((Hit2:GetAttribute("PenCo")*distance)/(WeaponData.BulletPenetration - Bullet:GetAttribute("DamageMultiplier"))) * BulletMass)
					--					warn(-Bullet.AssemblyLinearVelocity*((Hit2:GetAttribute("PenCo")*distance)/(WeaponData.BulletPenetration - DamageMultiplier)) * BulletMass)
					CastRay(Bullet, secondPosition.Position)
					break
				end		
			end
			...
			if not recast then
				local DamageMultiplier = Bullet:GetAttribute("DamageMultiplier")
				...
				if FoundHuman == true and VitimaHuman.Health > 0 and WeaponData then
					...
					if HitPart.Name == "Head" or HitPart.Parent.Name == "Top" or HitPart.Parent.Name == "Headset" or HitPart.Parent.Name == "Olho" or HitPart.Parent.Name == "Face" or HitPart.Parent.Name == "Numero" then
						Evt.Damage:InvokeServer(WeaponTool, VitimaHuman, TotalDistTraveled, 1, WeaponData, ModTable, nil, nil, SKP_02, DamageMultiplier, Velocity.Magnitude)
					elseif HitPart.Name == "Torso" or HitPart.Name == "UpperTorso" or HitPart.Name == "LowerTorso" or HitPart.Parent.Name == "Chest" or HitPart.Parent.Name == "Waist" or HitPart.Name == "Right Arm" or HitPart.Name == "Left Arm" or HitPart.Name == "RightUpperArm" or HitPart.Name == "RightLowerArm" or HitPart.Name == "RightHand" or HitPart.Name == "LeftUpperArm" or HitPart.Name == "LeftLowerArm" or HitPart.Name == "LeftHand" then				
						Evt.Damage:InvokeServer(WeaponTool, VitimaHuman, TotalDistTraveled, 2, WeaponData, ModTable, nil, nil, SKP_02, DamageMultiplier, Velocity.Magnitude)
					elseif HitPart.Name == "Right Leg" or HitPart.Name == "Left Leg" or HitPart.Name == "RightUpperLeg" or HitPart.Name == "RightLowerLeg" or HitPart.Name == "RightFoot" or HitPart.Name == "LeftUpperLeg" or HitPart.Name == "LeftLowerLeg" or HitPart.Name == "LeftFoot" then
						Evt.Damage:InvokeServer(WeaponTool, VitimaHuman, TotalDistTraveled, 3, WeaponData, ModTable, nil, nil, SKP_02, DamageMultiplier, Velocity.Magnitude)		
					end	
				end
...
function CreateBullet()
	...
	if WeaponData.BackBlast then
		Evt.BackBlast:FireServer(WeaponTool, WeaponData)
	end
...
function meleeCast()
	...
	if HitPart.Name == "Head" or HitPart.Parent.Name == "Top" or HitPart.Parent.Name == "Headset" or HitPart.Parent.Name == "Olho" or HitPart.Parent.Name == "Face" or HitPart.Parent.Name == "Numero" then
		Thread:Spawn(function()
			Evt.Damage:InvokeServer(WeaponTool, VitimaHuman, 0, 1, WeaponData, ModTable, nil, nil, SKP_02, 999999, nil)	
		end)
	...
	elseif HitPart.Name == "Torso" or HitPart.Name == "UpperTorso" or HitPart.Name == "LowerTorso" or HitPart.Parent.Name == "Chest" or HitPart.Parent.Name == "Waist" or HitPart.Name == "RightUpperArm" or HitPart.Name == "RightLowerArm" or HitPart.Name == "RightHand" or HitPart.Name == "LeftUpperArm" or HitPart.Name == "LeftLowerArm" or HitPart.Name == "LeftHand" then
		Thread:Spawn(function()
			Evt.Damage:InvokeServer(WeaponTool, VitimaHuman, 0, 2, WeaponData, ModTable, nil, nil, SKP_02, 999999, nil)	
		end)
	...
	elseif HitPart.Name == "Right Arm" or HitPart.Name == "Right Leg" or HitPart.Name == "Left Leg" or HitPart.Name == "Left Arm" or HitPart.Name == "RightUpperLeg" or HitPart.Name == "RightLowerLeg" or HitPart.Name == "RightFoot" or HitPart.Name == "LeftUpperLeg" or HitPart.Name == "LeftLowerLeg" or HitPart.Name == "LeftFoot" then
		Thread:Spawn(function()
			Evt.Damage:InvokeServer(WeaponTool, VitimaHuman, 0, 3, WeaponData, ModTable, nil, nil, SKP_02, 999999, nil)	
		end)
	end;		
end;
...
function UpdateGui()
	...
	if WeaponData.MagCount and WeaponData.ShootType ~= 6 then
		...
		elseif typeof(StoredAmmo) == "table" then
			HUD.SAText.Text = math.ceil(#StoredAmmo)
			HUD.Magazines.Visible = true
			HUD.Bullets.Visible = false						
		end
	elseif WeaponData.ShootType == 6 then
		HUD.SAText.Text =  WeaponTool:GetAttribute("GLAmmo")	
...
function Reload()
	print("Reloading!")
	if WeaponData.Type == "Gun" and (Ammo < WeaponData.Ammo or WeaponData.IncludeChamberedBullet and Ammo < WeaponData.Ammo + 1) and WeaponData.ShootType ~= 6 then
		if typeof(StoredAmmo) == "table" then
			if #StoredAmmo <= 1 then
				return
			end
		elseif typeof(StoredAmmo) == "number" then
		...
		elseif Ammo ~= nil and typeof(StoredAmmo) == "number" then
			if (((Ammo > 0 and not WeaponData.IncludeChamberedBullet) or (Ammo > 1 and WeaponData.IncludeChamberedBullet))) and not QuickReload then
				ReloadAnim()
			elseif QuickReload and (((Ammo > 0 and not WeaponData.IncludeChamberedBullet) or (Ammo > 1 and WeaponData.IncludeChamberedBullet))) then
				QuickReloadAnim()
			else
				TacticalReloadAnim()
			end
			if (((Ammo - (WeaponData.Ammo - StoredAmmo)) < 0 and not QuickReload) or ((StoredAmmo - (WeaponData.Ammo)) < 0 and QuickReload)) and Ammo ~= nil then
			...
			elseif QuickReload then
				if Ammo > 0 and WeaponData.IncludeChamberedBullet then
					StoredAmmo = StoredAmmo - WeaponData.Ammo
		...
		elseif Ammo ~= nil and typeof(StoredAmmo) == "table" then
			if (((Ammo > 0 and not WeaponData.IncludeChamberedBullet) or (Ammo > 1 and WeaponData.IncludeChamberedBullet))) and not QuickReload then
				ReloadAnim()
			elseif QuickReload and (((Ammo > 0 and not WeaponData.IncludeChamberedBullet) or (Ammo > 1 and WeaponData.IncludeChamberedBullet))) then
				QuickReloadAnim()
			...
			if not (((Ammo > 0 and not WeaponData.IncludeChamberedBullet) or (Ammo > 1 and WeaponData.IncludeChamberedBullet))) and Ammo ~= nil then
				local tempAmmo = Ammo
				local currentMag = WeaponData.CurrentMagazine
				table.remove(StoredAmmo, currentMag)
				local numberOfMags = #StoredAmmo
				if(currentMag > numberOfMags) then
					currentMag = 1									
				end
				Ammo = StoredAmmo[currentMag]

				WeaponData.CurrentMagazine = currentMag
			elseif (((Ammo > 0 and not WeaponData.IncludeChamberedBullet) or (Ammo > 1 and WeaponData.IncludeChamberedBullet))) and Ammo ~= nil and QuickReload then
				local tempAmmo = 0
				local currentMag = WeaponData.CurrentMagazine
				table.remove(StoredAmmo, currentMag)
				local numberOfMags = #StoredAmmo
				...
				if WeaponData.IncludeChamberedBullet then
					Ammo = StoredAmmo[currentMag] + 1
				else
					Ammo = StoredAmmo[currentMag]					
				end

				--		StoredAmmo[initialMag] = tempAmmo
				WeaponData.CurrentMagazine = currentMag

			elseif Ammo > 0 and Ammo ~= nil then
				if WeaponData.IncludeChamberedBullet then
					Ammo = Ammo - 1
					local tempAmmo = Ammo
					local currentMag = WeaponData.CurrentMagazine
					local initialMag = currentMag
					local numberOfMags = #StoredAmmo
					if(currentMag == numberOfMags) then
						currentMag = 1					
					else
...
function LauncherFx()
	...
	if not WeaponData.BackBlast then
		return
	end

	local BackBlast = WeaponInHand.Handle.BackBlast
	...
	WeaponInHand.Handle.BackBlast["FlashFX[Flash]"]:Emit(10)

	WeaponInHand.Handle.BackBlast["Smoke"]:Emit(10)
end
function GunFx()
	...
	if Suppressor and Muzzle:FindFirstChild("Suppressed") then
	...
	if Suppressor and Muzzle:FindFirstChild("EchoSuppressed") then

	elseif Muzzle:FindFirstChild("Echo") and not Suppressor then
...
function QuickReloadAnim()
	local tr = math.random(35, 45)
	local RecoilZ = math.rad(tr * RAND(-1, 1, .1))

	--	Evt.GunStance:FireServer("Quick", AnimData, WeaponTool, reloadStage)
...
Run.RenderStepped:Connect(function(step)
	...
	if ViewModel and LArm and RArm and WeaponInHand then --Check if the weapon and arms are loaded
		...
		if BipodAtt then
			...
			if BipodHit then
				...
				if CanBipod and BipodActive and not runKeyDown and (GunStance == 0 or GunStance == 2) then
					TS:Create(SE_GUI.GunHUD.Att.Bipod, TweenInfo.new(.1,Enum.EasingStyle.Linear), {ImageColor3 = Color3.fromRGB(255,255,255), ImageTransparency = .123}):Play()
					staminaValue = 0
					gunbobcf = gunbobcf * CFrame.Angles( math.rad(staminaValue * math.sin( tick() * 2 )),math.rad(staminaValue * math.sin( tick())), 0)				
					if not aimming then
						BipodCF = BipodCF:Lerp(CFrame.new(0,(((UnderBarrelAtt.Main.Position - BipodPos).magnitude)-1) * (-1.5), 0),.2)
					else
						BipodCF = BipodCF:Lerp(CFrame.new(),.2)
					...
					if breathHolding then
						...
						staminaValue = WeaponData.SwayBase + (1-(ACS_Client.Variaveis.Stamina.Value/ACS_Client.Variaveis.Stamina.MaxValue))* WeaponData.MaxSway					
			...
			gunbobcf = gunbobcf * CFrame.Angles( math.rad(staminaValue * math.sin( tick() * 2 )),math.rad(staminaValue * math.sin( tick() * 1 )), 0)
		end
		...
		if char.HumanoidRootPart then 
			...
			relativeVelocity = Vector3.new(velocity:Dot(root.CFrame.RightVector), velocity:Dot(root.CFrame.UpVector), velocity:Dot(root.CFrame.LookVector))
		end

		if velocity then
			recoilcf = recoilcf * CFrame.new(-0.02 * relativeVelocity.X / gameRules.RunWalkSpeed,-0.03 * relativeVelocity.Y / gameRules.RunWalkSpeed,0.05 * relativeVelocity.Z / gameRules.RunWalkSpeed)					
		end
		...
		if running and runKeyDown then
			gunbobcf = gunbobcf:Lerp(CFrame.new(
				0.015 * (Humanoid.WalkSpeed/gameRules.NormalWalkSpeed) * math.sin(tick() * 10),
				0.015 * (Humanoid.WalkSpeed/gameRules.NormalWalkSpeed) * math.cos(tick() * 5),
				0
				) * CFrame.Angles(
					math.rad( 3 * math.sin(tick() * 14 * Humanoid.WalkSpeed/gameRules.RunWalkSpeed) ), 
					math.rad( 25 * math.cos(tick() * 8 * Humanoid.WalkSpeed/gameRules.RunWalkSpeed) ), 
					math.rad(0)
				), 0.1)
		elseif running and not aimming then
			gunbobcf = gunbobcf:Lerp(CFrame.new(
				0.005 * (Humanoid.WalkSpeed/gameRules.NormalWalkSpeed) * math.sin(tick() * 5),
				0.005 * (Humanoid.WalkSpeed/gameRules.NormalWalkSpeed) * math.cos(tick() * 10),
				0
				) * 
					CFrame.Angles(
						math.rad( 1 * (Humanoid.WalkSpeed/30) * math.sin(tick() * 16) ), 
						math.rad( 1 * (Humanoid.WalkSpeed/30) * math.cos(tick() * 8) ), 
						math.rad( 2 * (Humanoid.WalkSpeed/15) * math.cos(tick() * 8) )
					), 0.1)
		elseif running and aimming then
			gunbobcf = gunbobcf:Lerp(CFrame.new(
				0.025 * (Humanoid.WalkSpeed/gameRules.NormalWalkSpeed) * math.sin(tick() * 8),
				0.025 * (Humanoid.WalkSpeed/gameRules.NormalWalkSpeed) * math.cos(tick() * 16),
				0
				) * CFrame.Angles(
					math.rad( 1 * (Humanoid.WalkSpeed/10) * math.sin(tick() * 16) ), 
					math.rad( 1 * (Humanoid.WalkSpeed/10) * math.cos(tick() * 8) ), 
					math.rad(0)
				), 0.1)		
		else
			gunbobcf = gunbobcf:Lerp(CFrame.new(
		...
		local Raio = Ray.new(WeaponInHand.Handle.Position, cam.CFrame.LookVector * gunSize)
		...
		if CurAimpart and aimming and AnimDebounce and not CheckingMag then
			if not NVG or WeaponInHand.AimPart:FindFirstChild("NVAim") == nil then
				if Hit and BipodActive == false then
					local ogComp = math.abs(((WeaponInHand.Handle.Position - Pos).magnitude/gunSize) - 1)
					local wallComp = ogComp

					local rotationAngle = 0
					if not runKeyDown then
						wallComp = math.min(ogComp, 0.03)
						rotationAngle = math.min(math.acos((gunSize - ogComp)/(gunSize - wallComp)) * 3, math.rad(90))	
					end
					if rotationAngle > math.rad(10) and WeaponData.Type == "Gun" and not runKeyDown then
						--canADS = false
						if aimming then
							--							aimming = false
							--							ADS(aimming)							
					...
					elseif WeaponData.Type == "Gun" then
						canADS = true
					end
					if AimPartMode == 1 then
						TS:Create(cam,AimTween,{FieldOfView = ModTable.ZoomValue}):Play()							
						maincf = maincf:Lerp(maincf * CFrame.new(0,0, .5) * recoilcf * Sway:inverse() * CurAimpart.CFrame:toObjectSpace(cam.CFrame) *  CFrame.new(0,0,wallComp)  * CFrame.Angles(0,math.max(rotationAngle,0),0) , 0.2)
					else
				...
				TS:Create(cam,AimTween,{FieldOfView = 75}):Play()
				if Hit and BipodActive == false then
					local ogComp = math.abs(((WeaponInHand.Handle.Position - Pos).magnitude/gunSize) - 1)
					local rotationAngle = 0
					local wallComp = ogComp
					...
					maincf = maincf:Lerp(maincf * CFrame.new(0,0, .5) * recoilcf * Sway:inverse() * (WeaponInHand.AimPart.CFrame * WeaponInHand.AimPart.NVAim.CFrame):toObjectSpace(cam.CFrame) *  CFrame.new(0,0,wallComp)  * CFrame.Angles(0,math.max(rotationAngle,0),0) * CFrame.Angles(0,math.max(rotationAngle,0),0), 0.2)										
				else
					canADS = true
			...
			if Hit and BipodActive == false then
				...
				maincf = maincf:Lerp(AnimData.MainCFrame * CFrame.new(0,0, .5) * recoilcf * Sway:inverse() *  CFrame.new(0,0,wallComp)  * CFrame.Angles(0,math.max(rotationAngle,0),0),0.2)					
			else
				canADS = true
	...
	if breathHolding  and not breathDebounce then
		ACS_Client.Variaveis.Stamina.Value -= gameRules.BreathValue			
		coroutine.resume(coroutine.create(function()
			breathDebounce = true
			wait(0.33)
	...
	if (gameRules.RunWalkSpeed - Humanoid.WalkSpeed > 0 and not breathHolding) and not staminaDebounce and not ACS_Client:GetAttribute("Injured") then
		coroutine.resume(coroutine.create(function()
			wait(5)
			if (not runKeyDown or not breathHolding or not script.Parent:GetAttribute("Injured")) then
				if Proned then
					ACS_Client.Variaveis.Stamina.Value += gameRules.ProneRecover		
				elseif Crouched then
					ACS_Client.Variaveis.Stamina.Value += gameRules.CrouchRecover
				elseif Steady or not running then
					ACS_Client.Variaveis.Stamina.Value += gameRules.StandRecover					
				else
					ACS_Client.Variaveis.Stamina.Value += gameRules.WalkRecover					
				end			
			end
	...
	if ACS_Client.Variaveis.Stamina.Value < 100 and runKeyDown then
		Stand()
		if not CheckingMag and not reloading and WeaponData and WeaponData.Type ~= "Grenade" and (GunStance == 0 or GunStance == 2 or GunStance == 3) then
			GunStance = 0
			Evt.GunStance:FireServer(GunStance,AnimData)
	...
	elseif char:WaitForChild("Humanoid").WalkSpeed < gameRules.RunWalkSpeed and runKeyDown and not aimming and running and not script.Parent:GetAttribute("Injured") and ACS_Client.Variaveis.Stamina.Value > 100 then
		mouse1down = false		
		Stand()
		Stances = 0
		Virar = 0
	...
	elseif ACS_Client.Variaveis.Stamina.Value <= 0 and breathHolding then
		breathHolding = false
		TS:Create(cam,AimTween,{FieldOfView = cam.FieldOfView + 5}):Play()	
	end	
end)
...
function onSeated(isSeated, seat)
	if isSeated then
		...
		CAS:UnbindAction("PlayerInput")
		print("I'm now sitting on: " .. seat.Name .. "Seat of Vehicle Here")
	else
		...
		CAS:BindAction("Interact", handleAction, false, Enum.KeyCode.G)
